...
class WalletDlg(QDialog, ui_send_payout_dlg.Ui_SendPayoutDlg, WndUtils):
    ...
    def setupUi(self):
        ...
        if len(self.masternodes) > 1:
            self.cbo_src_masternodes.addItem('<All MUEnodes>')
    ...
    def on_btnSend_clicked(self):
        ...
        if len(utxos):
            ...
            for utxo_idx, utxo in enumerate(utxos):
                ...
                if utxo['collateral']:
                    if self.queryDlg(
                            "Warning: you are going to transfer MUEnode's collateral (500000 MUE) transaction "
                            "output. Proceeding will result in broken MUEnode.\n\n"
                ...
                if addr_hw != utxo['address']:
                    self.errorMsg("<html style=\"font-weight:normal\">MUE address inconsistency between UTXO (%d) and HW path: %s.<br><br>"
                                 "<b>HW address</b>: %s<br>"
        ...
        def apply_utxos(new_utxos: List[dict], existing_utxos: List[dict]):
            ...
            if self.utxo_src_mode == 1:
                if self.mn_src_index is not None and self.mn_src_index < len(self.masternode_addresses) and \
                        self.mn_src_index >= 0:
                    msg = f'<b>Balance:</b> {self.sel_addresses_balance} MUE&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} MUE&nbsp;&nbsp;&nbsp;<b>Address:</b> {self.masternode_addresses[self.mn_src_index][0]}'
                    self.set_message_2(msg)
                else:
                    msg = f'<b>Balance:</b> {self.sel_addresses_balance} MUE&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} MUE'
                    self.set_message_2(msg)
            elif self.utxo_src_mode == 2:
                msg = f'<b>Balance:</b> {self.sel_addresses_balance} MUE&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} MUE'
                self.set_message_2(msg)
            elif self.utxo_src_mode in (1, 3):
                msg = f'<b>Balance:</b> {self.sel_addresses_balance} MUE&nbsp;&nbsp;&nbsp;<b>Received:</b> {self.sel_addresses_received} MUE&nbsp;&nbsp;&nbsp;<b>Address:</b> {self.hw_src_address}'
                self.set_message_2(msg)
